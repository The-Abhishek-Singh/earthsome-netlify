...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed top-20 right-6 z-50 flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl transform transition-all duration-500 ease-out bg-white border ${
      type === 'success' 
      ...
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
        ...
        {type === 'success' ? <Check size={16} className="text-white" /> : <X size={16} className="text-white" />}
      </div>
...
const BestSellers = () => {
  ...
  useEffect(() => {
    const fetchBestSellers = async () => {
      try {
        ...
        const filtered = res.data.filter(product => product.badge?.toLowerCase() === 'best seller');
  ...
  const getBadgeColor = (badge) => {
    switch (badge?.toLowerCase()) {
      case 'best seller': return 'bg-emerald-500';
    ...
    <div className="w-full bg-gradient-to-br from-green-50 to-white py-16 px-4">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-12">
          ...
          {/* <button className="text-green-600 hover:text-green-800 transition-colors font-medium border-b border-green-300 hover:border-green-500">
            View all products →
          </button> */}
        </div>
            ...
            {products.map((product) => (
              ...
              >
                ...
                <div className="p-6 relative z-20">
                  ...
                  <div className="mb-3">
                    <StarRating rating={product.rating || 4} reviews={product.reviewsCount || 0} />
                  </div>
                  ...
                  <button 
                    className={`w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white py-3 px-6 rounded-full transition-all duration-300 font-semibold transform hover:scale-105 shadow-lg hover:shadow-xl ${
                      addingToCart === product._id ? 'from-green-700 to-green-800 scale-95' : ''
                    }`}
