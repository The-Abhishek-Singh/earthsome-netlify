...
//   // Load checkout data from cart (including coupon)
//   useEffect(() => {
...
//           // SECURITY CHECK: If a coupon is applied but user is not authenticated, clear it
...
//           // SECURITY CHECK: Validate session signature to prevent tampering
...
//           console.log(
//             "✅ Loaded coupon from cart with valid session:",
//             data.appliedCoupon
//           );
//           console.log(
//             "Cart total:",
...
//       }
//     }
//   }, [session]); // Add session as dependency to re-run when auth status changes
...
//   // Coupon related state - loaded from cart only (no application allowed here)
//   const [appliedCoupon, setAppliedCoupon] = useState(null);
//   const [couponDiscount, setCouponDiscount] = useState(0);

//   // Function to get the correct total (with discount applied)
//   const getOrderTotal = () => {
//     const checkoutData = localStorage.getItem("checkoutData");
//     if (checkoutData) {
//       try {
//         const data = JSON.parse(checkoutData);
//         // Use the pre-calculated total from cart which includes discounts
//         return (
//           data.total ||
...
//       }
//     }
//     // Fallback: calculate from cart items minus coupon discount
//     return (
//       cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0) -
//       couponDiscount
//     );
...
//   // Function to verify coupon is still available before marking as used
...
//   // Function to mark coupon as used after successful order completion
...
//   // NOTE: Coupon application is ONLY allowed in the cart page with proper authentication
//   // This ensures all security measures and usage tracking are properly enforced
//   // The ShippingForm only displays coupons that were already applied in the cart
...
//                   // Double-check coupon availability before marking as used
...
//     try {
//       // Get the correct total with discount applied
...
//       // Note: We don't mark the coupon as used here - it will be marked after successful order completion

//       // Create order first
//       try {
//         // Determine if this is a guest order based on session
//         const isGuestOrder = !session || !session.user;
//         const isAuthenticatedUser = !isGuestOrder; // Clear boolean for authentication

//         // Prepare session data for authenticated users
...
//           sessionUserData: sessionUserData ? "Present" : "Not Present",
//           authenticationStatus: isAuthenticatedUser ? "AUTHENTICATED" : "GUEST",
...
//         // SECURITY CHECK: If coupon is applied, user MUST be authenticated
...
//           originalTotal: originalTotal, // Keep original for reference
//           paymentMethod,
//           paymentStatus: "pending", // Both COD and Online payments start as pending
//           isGuestOrder, // Include session-based guest detection
//           isAuthenticatedUser, // Clear authentication flag
//           sessionUserData, // Include session data for authenticated users
...
//           ...orderPayload,
//           sessionUserData: orderPayload.sessionUserData
...
//               finalTotal
//             );
//             await handlePayment(order._id, finalTotal); // Use the discounted total for payment
//             // Don't redirect here - payment success handler will do it
//           } catch (error) {
...
//           }
//         } else {
//           // For COD orders, mark coupon as used and redirect immediately
//           if (appliedCoupon && session?.user) {
//             const userId = session.user.id || session.user._id;
...
//                   >
//                     <div
//                       className="w-6 h-6 rounded-full border-2 border-gray-300 peer-checked:border-green-500
