...
export default function ComboProductDetailPage() {
    ...
    const fetchComboOffer = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/offers/combo/${id}`,
          {
            cache: "no-store",
          }
        );
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `Failed to fetch combo offer: ${res.status} ${res.statusText}`
          );
        }
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const pricing = calculateComboPrice(
        comboOffer.comboProducts,
        comboOffer.discountPercentage
      );
  ...
  return (
    <div
      id="combo-page-top"
      className="min-h-screen bg-white mb-[900px] sm:mb-[700px] md:mb-[600px] lg:mb-[485px]"
    >
      <FloatingCart />
            ...
            {comboOffer.startDate && (
              <div className="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                ...
                <p className="text-sm text-yellow-700">
                  This combo offer is valid from{" "}
                  {new Date(comboOffer.startDate).toLocaleDateString()}
                  {comboOffer.endDate &&
                    ` to ${new Date(comboOffer.endDate).toLocaleDateString()}`}
                </p>
              </div>
            )}
          </div>
